...
export default function ApplyPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (job) { fd.set('job_id', job.id); fd.set('job_title', job.title) }
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '25fr 75fr', gap: '3rem', alignItems: 'start' }}>
          ...
          <aside style={{ position: 'sticky', top: '96px' }}>
            ...
            {job && (
              <div style={{ backgroundColor: '#ffffff', border: '1px solid #E2E0D8', borderRadius: '12px', padding: '1.25rem', marginBottom: '1rem' }}>
                <span style={{ fontFamily: 'var(--font-montserrat)', fontWeight: 700, fontSize: '0.6rem', letterSpacing: '0.1em', textTransform: 'uppercase', color: '#C9922B', display: 'block', marginBottom: '0.5rem' }}>Role Summary</span>
                <div style={{ fontFamily: 'var(--font-montserrat)', fontWeight: 600, fontSize: '0.875rem', color: '#0A1628', marginBottom: '0.25rem' }}>{job.location}</div>
                <div style={{ fontFamily: 'var(--font-inter)', fontSize: '0.75rem', color: 'rgba(10,22,40,0.5)' }}>Ref ID: {job.refId}</div>
              </div>
            )}

            <div style={{ backgroundColor: '#0A1628', borderRadius: '12px', padding: '1.25rem' }}>
              <p style={{ fontFamily: 'var(--font-inter)', fontSize: '0.8125rem', color: 'rgba(255,255,255,0.65)', lineHeight: 1.6, marginBottom: '0.875rem' }}>Join the Elite. Every application is reviewed by our executive search team within 72 hours.</p>
          ...
          <div>
            <div style={{ backgroundColor: '#ffffff', borderRadius: '16px', border: '1px solid #E2E0D8', padding: '2.5rem', boxShadow: '0 2px 12px rgba(10,22,40,0.04)' }}>
              {job && <h1 style={{ fontFamily: 'var(--font-montserrat)', fontWeight: 900, fontSize: '1.25rem', color: '#0A1628', marginBottom: '0.25rem', letterSpacing: '-0.01em' }}>{job.title}</h1>}
                ...
                {step === 2 && (
                  <div>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                      ...
                      <div><label style={labelStyle} htmlFor="current_role">Current / Last Role</label><input id="current_role" name="current_role" type="text" placeholder="e.g. Senior Fund Manager at XYZ Capital" style={inputStyle} /></div>
                      ...
                      <div><label style={labelStyle} htmlFor="cover_note">Cover Note</label><textarea id="cover_note" name="cover_note" rows={5} placeholder="What makes you the right candidate for this role?" maxLength={500} style={{ ...inputStyle, height: 'auto', padding: '0.75rem 1rem', resize: 'vertical' }} /></div>
                    </div>
                ...
                {step === 3 && (
                  <div>
                    ...
                    <label style={{ display: 'flex', alignItems: 'flex-start', gap: '0.75rem', cursor: 'pointer', marginBottom: '2rem' }}>
                      ...
                      <span style={{ fontFamily: 'var(--font-inter)', fontSize: '0.875rem', color: 'rgba(10,22,40,0.65)', lineHeight: 1.5 }}>I agree to the <a href="#" style={{ color: '#C9922B' }}>Privacy Charter</a> and <a href="#" style={{ color: '#C9922B' }}>Terms of Engagement</a></span>
                    </label>
